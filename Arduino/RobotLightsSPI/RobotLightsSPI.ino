...
/*
   SPI controlled NeoPixel driver

   Receives command packets over SPI and allows painting RGB values into NeoPixel strip.
...
// The SPI library only supports configuring the arduino as an SPI controller,
...
ISR(SPI_STC_vect) {
  ...
  // space is available.
  if(nextQueueTail == recvQueueHead) {
    return;
  }

  recvBuff[recvQueueTail] = SPDR;
  recvQueueTail = nextQueueTail;
}
...
boolean nextByte(uint8_t& b) {
  if(recvQueueHead == recvQueueTail) {
    return false;
  }

  b = recvBuff[recvQueueHead];
